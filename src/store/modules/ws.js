 ...
 * @LastEditTime: 2024-07-22 19:43:16
 */
/**
     ...
     setTimeout(() => {
      ...
      if (state.resultimageHeartbeatList && state.resultimageHeartbeatList.length > 0) {
        ...
        if (index >= 0) {
          ...
          const base64array =   state.resultimageHeartbeatList[index].base64array
          base64array.unshift(imageBase64);
          const uniquebase64array = new Set(base64array);
          state.resultimageHeartbeatList[index].base64array= Array.from(uniquebase64array);
          ...
          state.resultimageHeartbeatList[index].base64 =imageBase64; // 当前最新实时照片 base64
        } else {
        ...
        // 第一个 构造对象数组
        const imageBase64 =  'data:image/png;base64,'+ data.imageBase64
        state.resultimageHeartbeatList.push({ uavId: deviceID, imgarray: [data.url], array: [data], url: data.url,base64:imageBase64,base64array: [imageBase64] });
        index = 0;
      }
      if (isNowUav) {
        // 当前 分析结果的图片list
